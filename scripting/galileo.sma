...
public srv_handleStart()
{
	...
	if (startAction)
	{
		new nextMap[32];
		new nextMap2[32];
		new arglen = read_argv(1, nextMap, 31);
		new arglenn = read_argv(1, nextMap2, 31);
		...
		if (startAction == SRV_START_CURRENTMAP || startAction == SRV_START_NEXTMAP)
		{
			...
			new filenamem[256];
			formatex(filenamem, sizeof(filenamem)-1, "%s/info.dat", DIR_DATA);
		
			new filem = fopen(filenamem, "rt");
			
			if (filem) // !feof(file)
				fgets(filem, nextMap2, sizeof(nextMap2)-1);
		...
		{
			set_task(5.0, "harta1", 0, nextMap, arglen + 50);
		}
		else if (is_map_valid(nextMap2))
			set_task(5.0, "harta2", 0, nextMap2, arglenn + 50);
		else
		{
