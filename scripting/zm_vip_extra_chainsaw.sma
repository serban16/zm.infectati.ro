...
public plugin_init()
{
	...
	register_concmd("amx_drujba", "vreau_drujba", ADMIN_IMMUNITY, "- sunt un om jegos cu drujba" );
	register_concmd("amx_faradrujbe", "bubui_drujbe", ADMIN_ALL, "- sunt un om jegos cu drujba" );
	register_concmd("amx_rdrujba", "bubui_drujbe2", ADMIN_ALL, "- sunt un om jegos cu drujba" );
...
public vreau_drujba(id, itemid, level, cid)
{
	if(!cmd_access(id,level,cid,2))
		...
		if(get_pcvar_num(cvar_enable))
		{
				...
				// Client Print
				client_printcolor(id, "/g[CS]/y Atentie cu /gDrujba")
		...
		{
			client_printcolor(id, "/g[ZP]/y Drujba a fost dezactivata. /gContacteaza Adminul")
		}
}

public bubui_drujbe()
{
	for(new ei = 1; ei <= g_maxplayers; ei++)
	{
		if (!is_user_connected(ei))
			return;
		g_iHasChainsaw[ei] = false
		reset_user_knife(ei)
	}
}
public bubui_drujbe2(id)
{	
	new target[32]
	...
	console_print(id, "I-ai inlaturat drujba lui %s", player)
	client_printcolor(player, "/g[ZP]/y LASA DRUJBA. /gPoftim un cutit!")
}
/*================================================================================
 [Main Forwards]
=================================================================================*/
