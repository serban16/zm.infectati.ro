...
/
/
/			[ZP] Extra Item  Dual Mp5
/			  ( Broñ Tylko Dla Humanów ! )
/
/				 by DeVvILL
/
/
...
/			Weapon cost 25 ammo packs.
/
/
/
/	Cvar's :
/
/			zp_dualmp5_unlimitedammo	<1|0> - Nie Skoñczonoœæ Strza³ów Automatycznie na 1.
/			zp_dualmp5_doubledamage		<2|0> - Dual Mp5 Obrazenia x2 Automatycznie  .
/			zp_dualmp5_cost			"20"  - Koszt Mp5
/
/
/
/	Komendy :
/
/			To buy weapon in chat you can say :
/			say /Dual or say_team /Dual
/
/
/
/	;) :
/
/			DeVvILL  - Dual Mp5 Dla Humanow !
/
/
/	To Do :
/
/			1) Create better extra damage
/			2) Add extra kills and other stuff
/			3) Somehow change that dual mp5 only will appear when you buy via extra item menu or chat.
/
/
/
/	Changelog :
/
/			13/09/2008 - v0.1 - First Release
/			17/09/2008 - v0.2 - Fixed in extra item menu name,  Tryed to fix extra item select, Removed unnecessary code.
/			18/09/2008 - v0.3 - Tryed to fix extra item select again,  Removed unnecessary lines.
/			19/12/2008 - v0.5 - Removed some unnecessary lines and cvars, changed plugin, now Dual Mp5 can buy only via extra item menu.
/			22/12/2008 - v0.7 - Removed some cvars and other stuff, added possebility to buy through chat, removed fun module.
/			23/12/2008 - v0.7.2 - added event death message, fixed buy through chat.
*/
...
#define PLUGIN "[ZP] Extra : Dual MP5"
...
public plugin_init()
{
	...
	g_dualmp5_doubledamage = register_cvar("zp_dualmp5_doubledamage", "1.2")
	g_dualmp5_cost = register_cvar("zp_dualmp5_cost", "30")
	
	g_itemid_dual_mp5 = zv_register_extra_item(g_item_name,"o runda", get_pcvar_num(g_dualmp5_cost), ZP_TEAM_HUMAN)
	
	register_clcmd("say /dm5", "buy_dual")
	register_clcmd("say_team /dm5", "buy_dual")
...
public buy_dual(id)
{
	...
	if (g_hasDual[id])
	{
		client_print(id, print_chat, "[ZP] You already have Dual MP5")
		return PLUGIN_HANDLED
	}
	...
	{
		client_print(id, print_chat, "[ZP] You don't have enough ammo packs!", ammo_cost)
		return PLUGIN_HANDLED
	}
	...
	fm_give_item(id, "ammo_9mm")
	fm_give_item(id, "ammo_9mm")
	client_print(id, print_chat,"[ZP] You have bought a Dual MP5.")
...
public zv_extra_item_selected(player, itemid)
{
	if (itemid == g_itemid_dual_mp5)
	{
		...
		fm_give_item(player, "ammo_9mm")
		fm_give_item(player, "ammo_9mm")
		client_print(player, print_chat,"[ZP] You have bought a Dual MP5, have fun!")
	}
	return PLUGIN_CONTINUE
}
