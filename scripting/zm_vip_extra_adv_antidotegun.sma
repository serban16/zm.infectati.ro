...
public plugin_init() 
{
	...
	g_iItemID = zv_register_extra_item("Arma cu antidot", "o runda", antidotegun_ap_cost, ZV_TEAM_HUMAN)
}
...
public zv_extra_item_selected(id, itemid)
{
	...
	if(itemid == g_iItemID)
	{
		...
		if(g_bCvar_Enabled)
		{
			...
			{
				client_printcolor(id, "/g[ZP]/y Trebuie sa astepti sa inceapa runda.")
			...
			{
				client_printcolor(id, "/g[ZP]/y Nu poti sa cumperi arma cu antidot.")
				zp_set_user_ammo_packs(id, zp_get_user_ammo_packs(id) + antidotegun_ap_cost)
				return PLUGIN_CONTINUE
			}
			#endif
			...
			if(g_bHasAntidoteGun[id] && bAlreadyHasGalil)
			{
				...
				{
					client_printcolor(id, "/g[ZP]/y Arma ta cu antidot este plina.")
		...
		{
			// A message
			client_printcolor(id, "/g[ZP]/y Arma cu antidot a fost dezactivata. /gContacteaza Adminul")
