...
public __csdm_allow_option(id, num)
{
	...
	{
		log_error(AMX_ERR_NATIVE, "Numar invalid: %d", option)
		return 0
	}
...
public csdm_Init(const version[])
{
	if (version[0] == 0)
	{
		set_fail_state("Modul respawn nu a putut fi incarcat.")
		return
	}
}
...
public csdm_reload(id, level, cid)
{
	...
	{
		client_print(id, print_chat, "[Respawn] Configurarile au fost reincarcate.")
	} else {
		client_print(id, print_chat, "[Respawn] Nu au putut fi gasite configurarile.")
	}
...
public csdm_enable(id, level, cid)
{
	...
	csdm_set_active(1)
	client_print(id, print_chat, "Modul Respawn activat.")
...
public csdm_disable(id, level, cid)
{
	...
	csdm_set_active(0)
	client_print(id, print_chat, "Modul Respawn dezactivat.")
...
public say_respawn(id)
{
	...
	{
		client_print(id, print_chat, "[Respawn] Aceasta comanda este dezactivata!")
		return PLUGIN_HANDLED
	}
