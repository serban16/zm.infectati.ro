...
new g_item_name[] = "Arma cu foc"
new g_item_discription[] = "o runda"
new g_msgScoreInfo
...
g_ft_fuelvalue, g_ft_damage_dis, g_ft_splash_dis, g_ft_extra_ammo_packs, g_ft_extra_frags, g_ft_remove, cvar_allow_times
...
public plugin_init()
{
	...
	g_ft_cost = register_cvar("zp_ft_cost", "50")
	g_ft_fuelcost = register_cvar("zp_ft_fuelcost", "4")
	g_ft_fueltank = register_cvar("zp_ft_fueltank", "200")
	g_ft_fuelvalue = register_cvar("zp_ft_fuelvalue", "1")
	g_ft_damage = register_cvar("zp_ft_damage", "200")
	...
	cvar_allow_times = register_cvar("zp_allow_flamethrower_times", "1")
...
public zv_extra_item_selected(id, itemid)
{
	if (itemid == g_itemid_ft)
	{
		...
		{
			client_print(id, print_center, "[ZP] Mai intai trebuie sa astepti sa inceapa runda")
			zp_set_user_ammo_packs(id, zp_get_user_ammo_packs(id) + get_pcvar_num(g_ft_cost))
			return 0
		}
		if (zp_is_nemesis_round())
		{
			client_print(id, print_chat, "Nu ai voie sa cumperi arma cu foc in modul nemesis.")
			return 0
		}
		if(g_bought[id] >= get_pcvar_num(cvar_allow_times)) {
			client_print(id, print_center, "[ZP] Poti cumpara Arma cu foc doar odata pe mapa.")
			return ZV_PLUGIN_HANDLED
		}
		if (g_hasFlamethrower[id])
		{
			zp_set_user_ammo_packs(id, zp_get_user_ammo_packs(id) + get_pcvar_num(g_ft_cost))
			client_print(id, print_center, "[ZP] Ai deja o arma cu foc.")
...
public fw_PlayerSpawn_Post(id)
{
	...
	if ((get_pcvar_num(g_ft_remove) == 1) && (g_hasFlamethrower[id])) {
				g_FuelTank[id] = 0
				g_Activated[id] = 0
...
public show_fuel_percentage(id)
{
	...
	show_hudmessage(id, "Combustibil: %d%%", get_percent(g_FuelTank[id], get_pcvar_num(g_ft_fueltank)))
}
