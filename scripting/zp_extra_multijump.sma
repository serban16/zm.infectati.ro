...
new const g_item_name[] = {"Multi-Saritura(o runda)"};
const g_item_cost = 2;
new g_maxJumps = 5;
...
public plugin_init()
{
	...
	g_itemid_multijump = zp_register_extra_item(g_item_name, g_item_cost, ZP_TEAM_HUMAN & ZP_TEAM_ZOMBIE);
...
{
	if(!is_user_alive(id) || !g_multijumps[id]) return PLUGIN_CONTINUE
...
public zp_extra_item_selected(player, itemid)
{
	if (itemid == g_itemid_multijump){
		if (g_multijumps[player] < g_maxJumps || !g_maxJumps){
			g_multijumps[player]++;
			if (g_maxJumps)
				client_print(player, print_center, "Acum poti sarii de %d / %d ori in aer.", g_multijumps[player], g_maxJumps);
			else
				client_print(player, print_center, "Acum poti sarii de %d ori in aer.", g_multijumps[player]);
		}
		else
			client_print(player, print_center, "Nu poti sarii de mai mult de %d ori in aer!", g_maxJumps);
	}
}
