...
public plugin_init()
{
	...
	/* CVARS */
	cvar_dmgmultiplier = register_cvar("zp_player_goldendg_dmg_multiplier", "2")
	cvar_custommodel = register_cvar("zp_player_goldendg_custom_model", "1")
	cvar_goldbullets = register_cvar("zp_player_goldendg_gold_bullets", "1")
	cvar_uclip = register_cvar("zp_player_goldendg_unlimited_clip", "0")
	
	// Register The Plugin
	register_plugin("[ZP] Extra: Golden Deagle", "1.1", "AlejandroSk")
	// Register Zombie Plague extra item
	g_itemid = zp_register_extra_item("*GOLD WEAPON: Desert Eagle*", 30, ZP_TEAM_HUMAN)
	// Death Msg
...
{
    if ( is_user_connected(attacker) && is_valid_player( attacker ) && get_user_weapon(attacker) == CSW_DEAGLE && g_HasDg[attacker] )
    {
...
public zp_extra_item_selected(player, itemid)
{
	if ( itemid == g_itemid )
	{
		...
		client_print(player, print_chat, "[ZP] Ai cumparat un Deagle Auriu.")
		g_HasDg[player] = true;
	}
}
