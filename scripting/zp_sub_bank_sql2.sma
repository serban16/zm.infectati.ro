...
public client_disconnect(id)
{
	...
	rezultat = SQL_PrepareQuery(sql, "SELECT zm_banca FROM tz_members WHERE auth='%s';", name)
	SQL_Execute(rezultat);
	
	if (SQL_NumResults(rezultat)) {
		rezultat = SQL_PrepareQuery(sql, "UPDATE `tz_members` SET `zm_banca` = '%d' WHERE `auth` = '%s'" ,bankstorage[id] , name)
		SQL_Execute(rezultat);
	
	}
	SQL_FreeHandle(rezultat);
