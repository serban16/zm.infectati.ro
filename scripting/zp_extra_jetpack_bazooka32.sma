...
new cvar_jetpack, cvar_jetpackSpeed, cvar_jetpackUpSpeed, cvar_jetpackAcrate ,cvar_RocketDelay, cvar_RocketSpeed, cvar_RocketDmg, cvar_Dmg_range, cvar_fly_max_engery, cvar_fly_engery, cvar_regain_energy, g_item_jetpack, cvar_cal_time, cvar_buyindelay, cvar_oneround
...
public plugin_init() {
	...
	cvar_buyindelay = register_cvar("zp_jp_buy_before_modestart", "1")
...
public rk_sound(player) {
	...
	if (!rksound[player])
	{
		...
		client_print(player, print_center, "[Bazooka] Reincarcata si pregatita !!!")
		rksound[player] = true
	}
...
public zp_extra_item_selected(player, itemid){
	...
	if ((itemid == g_item_jetpack) && zp_is_assassin_round())
	{
		client_print(player, print_chat, "[ZP] Nu poti cumpara Jetpack + Bazooka in modul 'Asasin'.")
		return;
	}
	
	if (!zp_has_round_started() && get_pcvar_num(cvar_buyindelay))
	{
		client_print(player, print_chat, "[ZP] Trebuie mai intai sa astepti sa inceapa runda.")
		zp_set_user_ammo_packs(player, zp_get_user_ammo_packs(player) + JETPACK_COST)
