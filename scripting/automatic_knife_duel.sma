...
Challenge(challenger, challenged) {
	...
	client_print(challenger, print_chat, "L-ai provocat pe %s la un duel de cutite! Asteapta raspunsul in %d secunde...", challenged_name, DECIDESECONDS)
...
public timed_toolate() {
	if (g_challenging) {
		...
		client_print(0, print_chat, "%s nu a raspuns suficient de repede intrebarii lui %s pentru duelul de cutite...", challenged_name, challenger_name)
		CancelAll()
	}
}
...
public Announcement(id) {
	client_print(id, print_chat, "Cand tu si un inamic ati ramas singuri,il poti provoca la un duel de cutite lovind cu lama de 3 ori in perete.")
}
...
DeclineMsg() {
	...
	get_user_name(g_challenger, challenger_name, 31)
	get_user_name(g_challenged, challenged_name, 31)
	client_print(0, print_chat, "%s a refuzat duelul lui %s", challenged_name, challenger_name)
}
...
Accept() {
	...
	client_print(0, print_chat, "%s a acceptat provocarea la duel al lui %s!", challenged_name, challenger_name)
	g_knifeArena = true
