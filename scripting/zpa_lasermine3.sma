...
new const
	ENT_MODELS[]	= "models/v_aiurea_mine.mdl",
...
new const
	...
	CHATTAG[] 					= "[zm.infectati.ro]",
	STR_NOROUND[]				= "Runda inca nu a inceput.",
	STR_NOTACTIVE[] 			= "Laserele nu sunt active.",
	STR_DONTHAVEMINE[]			= "Nu mai ai lasermine.",
	STR_MAXDEPLOY[]				= "Echipa ta a plantat prea multe lasere.",
	STR_MANYPPL[]				= "Too many ppl on your team...",
	STR_PLANTWALL[]				= "Planteaza laser-ul pe perete.",
	STR_CBT[]					= "Echipa ta nu poate sa planteze lasere!",
	STR_HAVEMAX[]				= "Ai maximul de lasere.",
	STR_DELAY[]					= "Poti planta laser-ul peste",
	STR_SECONDS[]				= "secunde.",
	STR_STATE[]					= "Lasere:",
	STR_NOACCESS[]				= "Nu ai acces la aceasta comanda.",
	STR_VIP[]					= "Ai cumparat un laser.",
	STR_SURVROUND[]				= "Nu poti sa plantezi laser in modul 'Supravietuitor'.",
	STR_NEMROUND[]				= "Nu poti sa plantezi laser in modul 'Nemesis'.",
	STR_ASSROUND[]				= "Nu poti sa plantezi laser in modul 'Asasin'.",
	STR_SNIPERROUND[]			= "Nu poti sa plantezi laser in modul 'Sniper'.",
	STR_PLAGUEROUND[]			= "Nu poti sa plantezi laser in modul 'Plague'.",
	STR_ARMAROUND[]				= "Nu poti sa plantezi laser in modul 'Armaghedon'.",
	STR_ASSASSIN_VS_SNIPER[]	= "Nu poti sa plantezi laser in modul 'Asasin vs Sniper'.";

new g_EntMine;
new beam, boom
new 
	g_LENABLE,g_AMMOKILL,g_LAMMO,g_LDMG, g_ITEMID, g_LCOST,
	g_LTMAX,g_LHEALTH,g_LMODE,g_LRADIUS,g_LRDMG,g_LFF,g_LCBT;
new
	g_LDELAY,/*g_LTHINK,*/g_LVISIBLE,
	g_LSTAMMO,g_LACCESS,g_LGLOW,g_LDMGMODE,g_LCLMODE,g_LCBRIGHT,g_LDSEC,g_LCMDMODE;

new g_dcount[33],g_nowtime,g_MaxPL
new bool:g_settinglaser[33]
new g_msgDeathMsg,g_msgScoreInfo,g_msgDamage,g_msgStatusText, g_NOROUND, g_NEMROUND, g_SURVROUND, g_ASSROUND, g_SNIPERROUND, g_ARMAROUND, g_PLAGUEROUND, g_ASSASSIN_VS_SNIPER, g_autobindlaser ;
...
public plugin_init() {
	...
	g_LENABLE				= register_cvar("zpa_lm","1");
	g_LACCESS				= register_cvar("zpa_lm_acs","0");
	g_LMODE					= register_cvar("zpa_lm_mode","0");
	g_LAMMO					= register_cvar("zpa_lm_ammo","2");
	g_LDMG					= register_cvar("zpa_lm_dmg","90000");
	g_LCOST					= register_cvar("zpa_lm_vip_cost","2");
	g_AMMOKILL				= register_cvar("zpa_lm_fragammo","2");
	g_LHEALTH				= register_cvar("zpa_lm_viata","1000");
	g_LTMAX					= register_cvar("zpa_lm_echipamax","40");
	g_LRADIUS				= register_cvar("zpa_lm_radius","320.0");
	g_LRDMG					= register_cvar("zpa_lm_rdmg","100");
	g_LFF					= register_cvar("zpa_lm_ff","0");
	g_LCBT					= register_cvar("zpa_lm_cbt","ALL");
	g_LDELAY				= register_cvar("zpa_lm_delay","0");
	g_LVISIBLE				= register_cvar("zpa_lm_line","1");
	g_LGLOW					= register_cvar("zpa_lm_sclipire","1");
	g_LCBRIGHT				= register_cvar("zpa_lm_bright","255");
	g_LCLMODE				= register_cvar("zpa_lm_culoare","0");
	g_LDMGMODE				= register_cvar("zpa_lm_ldmgmode","0");
	g_LDSEC					= register_cvar("zpa_lm_ldmgseconds","1");
	g_LSTAMMO				= register_cvar("zpa_lm_startammo","2");
	g_LCMDMODE				= register_cvar("zpa_lm_cmdmode","1");
	g_autobindlaser			= register_cvar("zpa_lm_autobind","1");
	g_NOROUND				= register_cvar("zpa_lm_noround","1");
	g_NEMROUND				= register_cvar("zpa_lm_nemround","1");
	g_SURVROUND				= register_cvar("zpa_lm_survround","1");
	g_ASSROUND				= register_cvar("zpa_lm_assassinround","1");
	g_SNIPERROUND			= register_cvar("zpa_lm_sniperround","1");
	g_PLAGUEROUND			= register_cvar("zpa_lm_plagueround","1");
	g_ARMAROUND				= register_cvar("zpa_lm_armaround","1");
	g_ASSASSIN_VS_SNIPER	= register_cvar("zpa_lm_assasin_vs_sniper","1");

	register_event("DeathMsg", "DeathEvent", "a");
 	register_event("CurWeapon", "standing", "be", "1=1");
	register_event("ResetHUD", "delaycount", "a");
	register_event("ResetHUD", "newround", "b");
	g_ITEMID = zv_register_extra_item("Laser", "o runda", 10, ZV_TEAM_HUMAN)
...
public plugin_cfg()
{
	...
	format(file, 63, "%s/zpa_lm_cvars.cfg", file);
...
{
	if(get_pcvar_num(g_autobindlaser) == 1)
	{
...
bool:CanCheck(id,mode)
{
	...
	{
		client_print(id, print_chat, "%s %s",CHATTAG,STR_NEMROUND);
		return false;
	}

	if(zp_is_survivor_round() && get_pcvar_num(g_SURVROUND))
	{
		client_print(id, print_chat, "%s %s",CHATTAG,STR_SURVROUND);
		return false;
	}

	if(zp_is_assassin_round() && get_pcvar_num(g_ASSROUND))
	{
		client_print(id, print_chat, "%s %s",CHATTAG,STR_ASSROUND);
		return false;
	}

	if(zp_is_sniper_round() && get_pcvar_num(g_SNIPERROUND))
	{
		client_print(id, print_chat, "%s %s",CHATTAG,STR_SNIPERROUND);
		return false;
	}
	
	if(zp_is_plague_round() && get_pcvar_num(g_PLAGUEROUND))
	{
		client_print(id, print_chat, "%s %s",CHATTAG,STR_PLAGUEROUND);
		return false;
	}
	
	if(zp_is_lnj_round() && get_pcvar_num(g_ARMAROUND))
	{
		client_print(id, print_chat, "%s %s",CHATTAG,STR_ARMAROUND);
		return false;
	}
	if(zp_is_avs_round() && get_pcvar_num(g_ASSASSIN_VS_SNIPER))
	{
		client_print(id, print_chat, "%s %s",CHATTAG,STR_ASSASSIN_VS_SNIPER);
		return false;
	}
	...
	while((ent = engfunc(EngFunc_FindEntityInSphere, ent, vecSrc, Radius)) != 0)
	{
		...
		if (flFraction >= 0.9 || get_tr2(trRes, TR_pHit) == ent)
		{
			...
			if(iHitHP <= 0)
			{
				if(iHitTeam != TeamID)
				{
					zp_set_user_ammo_packs(AtkID,zp_get_user_ammo_packs(AtkID) + get_pcvar_num(g_AMMOKILL))
	...
	if((pev(isHit, pev_flags) & (FL_CLIENT | FL_FAKECLIENT | FL_MONSTER)))
	{
		...
		if(iHitHP <= 0)
		{
			if(iHitTeam != teamid)
			{
				...
				hitscore = 1
				zp_set_user_ammo_packs(id,zp_get_user_ammo_packs(id) + get_pcvar_num(g_AMMOKILL))
...
public zv_extra_item_selected(player, itemid)
{
	...
	if (itemid == g_ITEMID)
	{
		...
		g_havemine[id]++;
		emit_sound(id, CHAN_ITEM, ENT_SOUND5, VOL_NORM, ATTN_NORM, 0, PITCH_NORM);
		client_print(id, print_chat, "%s %s",CHATTAG,STR_VIP);
	}
