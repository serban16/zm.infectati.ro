...
public plugin_init()
{
	...
	register_clcmd("say /arme", "enableMenu")
...
public m_EquipHandler(id, menu, item)
{
	...
	switch(choice)
	{
		...
		case 3:
		{
			...
			client_print(id, print_chat, "[CSDM] Scrie in chat /arme pentru a re-activa meniu de arme.")
			equipUser(id, EQUIP_ALL)
		}
	}
...
{
	//Equip Menu
	menu_additem(g_EquipMenuID, "Arme noi", "1", 0, -1)
	menu_additem(g_EquipMenuID, "Selectia anterioara", "2", 0, -1)
	menu_additem(g_EquipMenuID, "Tine-ma minte", "3", 0, -1)
	menu_setprop(g_EquipMenuID, MPROP_EXIT, MEXIT_NEVER)
	
	//Armor Menu
	menu_additem(g_ArmorMenuID, "Cu Armura", "1", 0, -1)
	menu_additem(g_ArmorMenuID, "Fara armura", "2", 0, -1)
	menu_setprop(g_ArmorMenuID, MPROP_EXIT, MEXIT_NEVER)
	
	//Nade Menu
	menu_additem(g_NadeMenuID, "Toate grenazile", "1", 0, -1)
	menu_additem(g_NadeMenuID, "Fara grenazi", "2", 0, -1)
...
public enableMenu(id)
{
	...
	{
		g_mShowuser[id] = true
		client_print(id, print_chat, "[Respawn] Meniu tau de arme a fost reactivat.")
		if (!g_MenuState[id])
		{
			g_MenuState[id] = 1
		...
		client_print(id, print_chat, "[Respawn] Meniu tau de arme este deja reactivat.")
	}
