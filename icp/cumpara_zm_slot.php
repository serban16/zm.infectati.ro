...
<?php require 'meniu.php'; ?>
			...
			if($_POST['submit']=='Da')
			{
				...
				if ($balanta >= $pret)
				{
					...
					$today = date("Y-m-d", time());
						...
						{
							$zm_access = "bcdefghijklmnopqrstu";
						}
						else if ($zm_access == "bcdefghijklmnopqrstux")
						{
							$zm_access = "bcdefghijklmnopqrstux";
						}
					...
					$Body = "";
					$Body .= "Ai cumparat pe Zm.infectati.ro Slot si iti expira pe data de ";
					$Body .= $expiry;					
					$Body .= "\n";
					...
					mysql_query("INSERT INTO log(comanda,admin,data) VALUES ('$auth a cumparat Slot pe zm.infectati.ro si a platit $pret Euro','$admin',NOW())");
		...
		<center>
			<h3>Esti sigur ca vrei sa cumperi Slot pe server-ul Zm.infectati.ro ?<br><br></h3>
